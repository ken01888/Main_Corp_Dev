...
const selectAllProduct = async (values) => Query('SELECT * FROM Recipe.Products', [values])
...
const insertInputItemMeta = async(total_cost = 0,total_input_count = 0,total_weight:string|number=0,id) => Query('UPDATE Recipe.Products SET total_cost = ?, total_input_count = ?, total_weight = ? WHERE id = ?',[total_cost, total_input_count,total_weight,id])
