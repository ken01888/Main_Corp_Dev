...
const getClientInformation = async (id) => Query('SELECT * FROM Credentials.Principle_Login_Credentials where id = ?',[id]);
const updatePRINCIPLE = async (info,id)=>Query('UPDATE Credentials.Principle_Login_Credentials SET ? WHERE id = ?',[info,id])
